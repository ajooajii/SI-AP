...
interface ActivationPendingViewProps {
  profile: UserProfile | null;
  onNotify: (type: "success" | "error" | "info" | "warning", message: string) => void;
  logActivity: any;
}

export function ActivationPendingView({ profile, onNotify, logActivity }: ActivationPendingViewProps) {
    ...
    try {
      ...
      if (user) {
        ...
        if (docSnap.exists()) {
          ...
          if (data.status === 'active' && data.role !== 'viewer') {
            onNotify('success', 'Akun Anda telah diaktifkan! Silakan tunggu halaman memuat kembali...');
            window.location.reload();
          } else {
            onNotify('error', 'Akun Anda masih dalam status menunggu aktivasi.');
          }
        } else {
          onNotify('error', 'Profil akun tidak ditemukan.');
        }
      }
    } catch (err) {
      console.error(err);
      onNotify('error', 'Gagal memeriksa status akun.');
    } finally {
      setChecking(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-slate-200">
      ...
      >
        <div className="flex justify-center">
          <div className="p-4 bg-amber-500/10 rounded-full text-amber-500 border border-amber-500/20 shadow-lg shadow-amber-500/5 animate-pulse">
            <AlertCircle className="w-12 h-12" />
          </div>
        </div>

        <div className="space-y-2">
          <h2 className="text-xl md:text-2xl font-bold text-white tracking-tight">Akun Anda sudah terdaftar.</h2>
          <p className="text-sm text-slate-400">Akun belum dapat menggunakan aplikasi sepenuhnya.</p>
        </div>

        <div className="p-4 bg-slate-950 rounded-xl border border-slate-800 text-left space-y-3">
          <p className="text-xs text-slate-400 leading-relaxed">
            Silakan hubungi operator Bidang PS untuk proses aktivasi akun.
          </p>
          {profile && (
            <div className="pt-2 border-t border-slate-800 text-[11px] space-y-1">
              ...
              <div className="flex justify-between"><span className="text-slate-500">UID:</span> <span className="font-mono text-slate-500 text-[9px] truncate max-w-[150px]">{profile.userId}</span></div>
          ...
          <button
            onClick={async () => {
              if (profile && logActivity) {
                await logActivity('login', 'logout', 'Autentikasi', 'Pengguna keluar dari sistem (Pending Screen)', { profile });
              }
              signOut(auth);
            }}
